...
> Platform: **GitHub**. Resolution path: GraphQL `resolveReviewThread` after a reply; **do not** use the GitHub UI "Resolve conversation" button — that path is intentionally disabled for review-thread-style comments.
...
**Reply on the thread** (GraphQL `addPullRequestReviewThreadReply`):
...
Resolving every thread is **necessary but not sufficient**. A review with state `CHANGES_REQUESTED` blocks merge independently of thread state via GitHub's standard review-verdict gate. Check it after the threads clear:
...
If `reviewDecision == "CHANGES_REQUESTED"`, the programmatic dismissal path is the GraphQL `dismissPullRequestReview` mutation. Find the bot's review first:
