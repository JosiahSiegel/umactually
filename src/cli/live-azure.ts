...
export async function runAzureLive(input: {
  ...
  // Post the inline threads FIRST so the parent's later POST gets the
  // highest thread id on the PR. The inline body reference to the
  // parent is left as a placeholder (no "Reply to PR review summary
  // #X" text yet) and is filled in via PATCH below.
...
async function patchInlineCommentWithParentRef(input: {
  ...
  readonly comment: LiveReviewComment | undefined;
