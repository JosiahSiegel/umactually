...
function collectAlwaysValidationErrors(parsed: ParsedCliArgs): readonly ValidationError[] {
  ...
  if (!parsed.dryRun) {
    ...
    if (
      parsed.apiKey === null ||
      (parsed.apiKey.length === 0 && parsed.provider !== "command")
    ) {
      errors.push({
        flag: "--api-key",
        message: "--api-key is required",
        hint: "Pass --api-key <key> or UMACTUALLY_API_KEY=<key> (use a CI secret, never source), or use --dry-run to skip the provider call. The `command` provider does not need an API key.",
      });
    }
  }
