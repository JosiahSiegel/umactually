...
- **`umactually doctor --suggested-config`** — after running the diagnostic checks, prints a copy-pasteable config suggestion for the highest-priority failing check (priority order: `api-key` > `api-url` > `provider-config` > `dist-freshness` > `node-version` > `git-context`). Read-only; never mutates state. Suggestion surfaces in `data.suggestion` under `--json`.
- **`umactually doctor --fix <check-id> [--yes]`** — opt-in, idempotent repair for failing checks. Supported check IDs: `dist-freshness` (runs `npm run bundle`), `provider-config` (prints the `init` command to run), `node-version` (no-op; surfaces the install hint). Each repair wrapped in try/catch; `exitCode: 2` for an unknown check-id, `exitCode: 1` for a thrown repair. Requires `--yes` or `UMACTUALLY_DOCTOR_FIX_YES=1` to run.
