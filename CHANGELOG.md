...
- **Resolution guides now lead with an open `## For AI agents` header (disposition matrix + reply/resolve/verify commands + review-verdict gate visible without expanding the `<details>` collapse)** so first-pass AI-agent scans catch the platform-specific protocol (GitHub GraphQL `resolveReviewThread`, Azure DevOps `az repos pr thread update --status closed`). Previously the entire AI-agent-facing guide lived inside a `<details>` block, and reviewers reading the bot's review body saw only a one-line summary — a real gap that caused PR #219 to receive a thread-resolution summary recommending the human-UI "Resolve conversation" button instead of the AI-agent `resolveReviewThread` mutation. The prepended header is purely additive; the existing `<details>` body content (Steps 1–6) is unchanged and still covered by `test/unit/self-review-noise-skip.test.ts`. The append-marker is bumped `<!-- umactually:resolution-guide-v1 -->` → `<!-- umactually:resolution-guide-v2 -->` so PRs with `v1` already appended get `v2` on the next self-review run (the marker-based idempotency check skips append when the literal is already in the body). `.github/SELF-REVIEW-RESOLUTION-GUIDE.md` (the standalone repo-root deep guide) gains a top-of-doc pointer to the AI-agent header so human readers find the same protocol first-pass. `[.github/workflows/data/resolution-guide-github.md] [.github/workflows/data/resolution-guide-azure.md] [.github/workflows/self-review.yml] [.github/SELF-REVIEW-RESOLUTION-GUIDE.md]`
...
- **Auditable first-class review contracts across the prior thirteen product tasks**: a committed review policy with four-tier provenance, a schema-versioned local review benchmark, durable finding fingerprints and collision safety, context provenance and safe base-branch instruction discovery, local privacy-preserving metrics, durable incremental review state machine, validated developer-controlled suggestions, incremental GitHub and Azure reconciliation, and a documented GitHub Enterprise Server endpoint path. Benchmark claims link to the exact reproduction command and artifact contract in [`docs/benchmark.md`](docs/benchmark.md); architecture, limitations, privacy, and rollback are mapped in [`docs/architecture.md`](docs/architecture.md). (PR #209)`
- **`umactually.review.json` committed policy surface** (schemaVersion 1) — strictly separate from `umactually.config.json` (the security boundary). New fields: `pathRules`, `excludes`, `effort` (`low|medium|high`), `triggers` (`opened|synchronize|reopened`), `reReviewCap`, `budgets` (`contextTokens/maxOutputTokens/latencyMs`), `minimumSeverity` (`info|warning|error`), `suggestionMode` (`off|validated`), `gateMode` (`off|warn|block`). 4-tier precedence (CLI > env > committed policy > defaults) with every resolved field carrying provenance (source/path/hash/version). Strict validation runs BEFORE any provider/platform call: unknown keys rejected, duplicate or conflicting path rules rejected, invalid globs rejected, unsafe paths rejected, secrets detected and rejected, unsupported schema versions rejected. Failures return exit code 2 and write NO files; error messages are redacted. Init's policy template is EXPLICITLY opt-in via `umactually init --policy-template` (default `umactually init` does NOT create or modify `umactually.review.json`). `umactually --show-config` prints the policy path/schemaVersion/hash and resolved fields when present; `umactually review --json` envelope's `resolvedConfig.reviewPolicy` carries sanitized `{ path, hash, schemaVersion }` for audit. `[src/cli/review-policy.ts] [src/cli/load-policy.ts] [src/cli/apply-policy.ts] [src/cli/serialize-policy.ts]` (PR #209)`
- **Public trust and contributor surfaces**: root [`SECURITY.md`](SECURITY.md), private-advisory/support issue routing, an architecture/contributor map, and a dependency-update policy. Deferred scope is explicit: no hosted control plane, GitLab, Bitbucket, opaque learning, or auto-commit behavior. (PR #209)`
- **`umactually doctor` now has a full-readiness mode** with bounded, safe diagnostics that operators can run ahead of a release to catch configuration drift before it surfaces as a CI failure. (PR #209)`
- **`Instruction-file auto-discovery now covers the full convention ecosystem`**: `DEFAULT_PROMPT_FILE_PATHS` expanded from 5 hand-picked filenames (`CLAUDE.md`, `AGENTS.md`, `.github/copilot-instructions.md`, `.cursorrules`, `GEMINI.md`) to 32 entries covering the full convention ecosystem — Tier 1 cross-tool (AGENTS.md variants, CLAUDE.md variants, GEMINI.md), Tier 2 single-tool (Copilot, Cursor, Cline, Roo, Kilo, Continue, Windsurf, OpenCode, JetBrains Copilot), Tier 3 directory modes (`.cursor/rules/*.mdc`, `.clinerules/**/*.md`, `.roo/rules/**/*.md`, `.github/instructions/*.instructions.md`, etc.), and human convention files (README.md, CONTRIBUTING.md, CODE_OF_CONDUCT.md, SECURITY.md, CHANGELOG.md, LICENSE with a 16 KiB per-file cap). Globs are expanded synchronously via a new `resolveGlobs` helper; per-file and aggregate byte caps remain enforced. `[src/config/prompt-files.ts]` (PR #195)`
...
- **CI lint now uses ESLint + typescript-eslint flat config** (the project adopted the v9 flat-config format). Onboarded via `eslint.config.mjs` at the repo root. Existing scoped `eslint-disable` comments were removed where the new config already accepts the construct, so the visible lint surface is unchanged for shipped code while the config is now declarative and version-stable. `[eslint.config.mjs]` (PR #205)`
- **`umactually init` keeps generated workflow pins aligned with `package.json` `version`**: the action version written to `examples/github/pr-review.yml` and `examples/azure/azure-pipelines.yml` is now read from the package at init time, not hand-pinned. The example files in the repo stay pinned to the current release via a fresh `init-templates-drift.test.ts` structural fingerprint + live `umactually@<version>` equality check, so a future bump that touches one file but not the other is caught in CI. (PR #209, PR #208)`
- **`--no-instruction-files` opt-out flag** (and `UMACTUALLY_INSTRUCTION_FILES=false` env var) disables the entire default-lookup for operators who want full control of the prompt contents. Off-by-default behavior is unchanged for `--prompt-files` / `--prompt-files` / `--additional-prompt-files` overrides. `[src/cli/parse-args.ts] [src/config/field-schema.ts]` (PR #195)`
- **README shrunk to a 9-section minimalist layout** (TL;DR + Quickstart + Install + Common operations + Provider + CI + Verify + Uninstall + Documentation + License; 119 lines) and CI now asserts the layout via `R-1..R-15` invariants. Old verbose prose was moved to focused docs under `docs/` and the example workflow pins were bumped to match the current release. `[README.md]` (PR #208)`
- **`.omo/` workflow artifacts are untracked from the repo and `git grep` decoupled**. The `.omo/` directory (used by the maintainers' release prep tooling) is now git-ignored the same way `.codegraph/` is, and the few `src/`/`test/`/`scripts/` doc-comment cross-references that pointed into it were inlined as standalone descriptions. Operators and CI no longer see the artifacts in the tracked tree; the maintainer tooling keeps writing them locally. `[.omo/.gitignore]` (PR #216)`
- **GitHub Actions pin floor raised.** `actions/setup-node`, `actions/github-script`, `actions/upload-artifact`, and `SonarSource/sonarqube-scan-action` were bumped to the now-current major versions (v7, v9, v7, v8.2.1 respectively) via dependabot. The bundled test fixtures that pinned the old majors were updated in lockstep. `[.github/workflows/]` (PR #211, #213, #214, #210, #217)`

### Fixed

- **`CliArgumentError` now extends `CliUsageError`**, so file-read errors in the CLI surface the same hint line ("hint: check the path and try again") as parse errors. Previously a malformed-path file read rendered the bare error without the hint, leaving operators one support-message poorer. Self-review follow-up tightened the integration tests. `[src/cli/errors.ts] [src/cli/dispatch.ts]` (PR #203)`
- **`fetchGithubPrInstructions` additional-prompt path was reading instruction files from the working tree even when the live path was already supplying the base-branch snapshot**. Switched to the base-branch-cached content so the additional prompt is always sourced from the trusted SHA, not the (potentially attacker-controlled) PR head. Hardened with a regression test that exercises the live-orchestrator additional-prompt path. `[src/provider-prompts.ts] [src/cli/orchestrator.ts]` (PR #201)`
- **Byte cap is now enforced on the joined base-branch instruction content** (not just per-file). Previously, any number of base-branch files that each passed the per-file cap could together exceed the aggregate budget, opening an unintended cost path. The current cap is the same per-file + aggregate contract documented for the cwd-default-lookup path. `[src/provider-prompts.ts]` (PR #200)`
- **`/umactually` self-review parse-fail guard now emits `::error::` annotations** (instead of a plain `parse failed` line) so the GitHub step summary surfaces the failure in the standard annotation UI and downstream SonarCloud comment-surfacing treats it as a definite failure. A regression test pins the new annotation shape. `[.github/workflows/self-review.yml]` (PR #197)`
- **`Verify npm publication` step in the release workflow is now a two-phase probe**, eliminating the false-negative timeout the v0.8.0 release cut exposed. Phase 1 fast-paths on the package-level `dist-tags.latest` signal (12 × 5s = 60s budget); Phase 2 cross-validates with a longer-budget per-version URL probe (60 × 10s = 600s). `[.github/workflows/release.yml]` (PR #194)`
- **`post-release-e2e.yml` `on.release.types` filter widened from `[published]` to `[published, released, prereleased]`**, so the post-release e2e workflow fires reliably for releases published via `gh release edit --draft=false` and for future prereleases, not just the initial draft-to-published transition. The existing `Resolve target tag` step reads `github.event.release.tag_name` from whichever event type fires, so no downstream changes were required. `[.github/workflows/post-release-e2e.yml]` (PR #194)`
- **`include-pr-sonar-findings` docs overclaimed the default in `configuration.md`**: the row's "Default" column says `false` and the v0.7.0 changelog explicitly states the default is `no`, but the description column carried a stale "(default in v0.8.0)" parenthetical. Removed so the description matches the default column and the published changelog. `[docs/configuration.md]` (PR #207)`
