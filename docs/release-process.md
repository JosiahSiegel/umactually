...
| `npm run render-docs` | The token contract was violated (a non-canonical `{{UMACTUALLY_*}}` token snuck in). | Read the script output for the file and token name. Replace it with `v0.6.10` or `0.4.0` as appropriate. Exit 2 means a typo; exit 1 means a token survived. |
