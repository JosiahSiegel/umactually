...
The per-archive transfer size check lives inline in `.github/workflows/release.yml`'s "Compute release-size report" step. It applies a `MIN_RAW_BYTES` floor (1 MiB; rejects empty / truncated / partial SEA blobs that would still pass a self-consistent sha256 but crash on launch) and a `MAX_RAW_BYTES` ceiling (200 MiB; the same safety net as the v0.5.x budget file). The check applies to the raw binary, not the archive — the archive is the gzip/zip wrapper, and a 130 MiB raw binary compresses to ~30 MiB transfer. The same sanity check is mirrored in `scripts/ci-release-pipeline-dry-run.sh` so the PR-time dry-run and the actual release build enforce one set of bounds. When a future target's expected size legitimately grows, bump the ceiling in BOTH the workflow and the dry-run script, and document the reason in the PR — do not silently widen the cap.
