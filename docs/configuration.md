...
| `debug-raw-response` | `UMACTUALLY_DEBUG_RAW` | `false` | `true`, `false` | Echo the raw provider response into the workflow log. The companion action exposes no debug input; to enable raw-response capture in CI, set job-level env `UMACTUALLY_DEBUG_RAW: "1"` on the review job (composite action steps inherit it). The var is read in the local CLI process and the value must be exactly `"1"`. |
