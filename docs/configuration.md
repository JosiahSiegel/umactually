...
Every GitHub Release under a `vX.Y.Z` tag ships exactly seven public assets. Six are compressed archives (`.tar.gz` on Linux/macOS, `.zip` on Windows), each wrapping one binary member; one is a SHA-256 manifest (`checksums.txt`). The manifest's source of truth is [`scripts/release-targets.json`](../scripts/release-targets.json); the workflow and the installers both parse it.
...
| `umactually-darwin-x64.tar.gz` | gzipped tar | `umactually-darwin-x64` | same |
...
| `checksums.txt` | manifest | — | generated by `scripts/verify-release-assets.mjs --measure` |
