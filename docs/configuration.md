...
| `model` | `UMACTUALLY_MODEL` | `auto` | `auto`, `review-model-synthetic` | `review-model-synthetic` is intended for fixtures and deterministic tests. `auto` is opinionated: it resolves to `gpt-5-mini` for OpenAI endpoints, `claude-sonnet-4.6` for Anthropic and Copilot, and `gemini-2.5-flash` for Google endpoints — each chosen for the lowest citation-hallucination rate among production code-review models per the Vectara HHEM 2026-05-11 leaderboard. Set a literal model name (`gpt-5-mini`, `claude-sonnet-4.6`, etc.) to override. |
...
| `verify-findings` | `UMACTUALLY_VERIFY_FINDINGS` | `true` | `true`, `false` | Deterministic re-verification of the model's `comments[]` against the supplied diff. Any comment whose (path, line) does not anchor is dropped before posting, regardless of the strict-schema outcome. Set to `false` via `--no-verify-findings` only if the caller has out-of-band validation. The same filter powers the `parse-warnings.json` artifact that records what was dropped. |
