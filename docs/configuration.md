...
**The shell and PowerShell installers are not CI-grade installation paths.** `scripts/install.sh` and `scripts/install.ps1` are convenience installers for interactive machines. CI must use Node 24 and a version-pinned install such as `npx github:JosiahSiegel/umactually#v0.4.0`; never curl an installer or resolve an unpinned latest version in a review pipeline.