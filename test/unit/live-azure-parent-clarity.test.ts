...
describe("buildReviewBody — 5-second scannable clarity", () => {
  ...
  it("CLARITY-14b: clean review (0 posted + 0 suppressed) collapses to the ship-it body", () => {
    ...
    expect(body).toContain(REVIEW_MARKER);
    expect(body).toContain("## ✅ 0 inline findings — ship it");
    // Clean reviews take the ship-it branch: no findings table or
    // summary `<details>` blocks. The single `<details>` block that
    // DOES render is the platform-aware resolution guide footer
    // (baked by `src/render/summary-layouts.ts` between the
    // Generated-by footer and the manifest). Count it explicitly
    // rather than blanket-asserting "no <details> anywhere" — that
    // pre-resolution-guide assertion was outdated by the
    // bake-resolution-guide plan and would block the guide from
    // rendering on ship-it reviews (where agents still need the
    // close-the-review protocol — see CHANGELOG.md:141 "a clean
    // review IS a review").
    expect((body.match(/<details>/gu) ?? []).length).toBe(1);
    expect(body).toContain("<!-- umactually:resolution-guide-v3 -->");
    expect(body).not.toMatch(/^Generated by/m);
