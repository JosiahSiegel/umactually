  ...
  it("parses the real 17KB review without firing the empty-review retry path", () => {
    ...
    const realReview = {
      ...
      comments: [
        ...
        { path: "test/unit/summary-layouts.test.ts", line: 428, body: "The cross-cutting invariants test on line ~427 has a hardcoded count string (📊 6 inline findings) — derive the expected string from the same data instead of hardcoding so it survives fixture changes.", severity: "medium", category: "correctness" },
      ],
      suppressed_comments: [
