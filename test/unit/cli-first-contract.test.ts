...
describe("CLI-first contract: every CLI flag has a field-schema entry", () => {
  ...
  it("action.yml declares every field-schema input that is meant to be user-tunable", async () => {
    // The CLI-first contract extends to the action surface: every
    // field-schema entry that has a non-empty input name MUST be
    // declared in action.yml. Internal-only fields (azure-org etc.)
    // are excluded because they're ADO-specific env-var reads
    // (handled by src/config/env-sources.ts), NOT user inputs.
    const yaml = await readFile(
      join(process.cwd(), "action.yml"),
      "utf8",
    );
    const actionInputs = new Set<string>();
    for (const m of yaml.matchAll(/^  ([a-z][a-z0-9-]*):/gum)) {
      const name = m[1];
      if (typeof name === "string") {
        actionInputs.add(name);
      }
    }
    ...
    const missing: string[] = [];
    for (const [field, def] of Object.entries(FIELDS) as [string, { input: string; flag: string | null }][]) {
      if (def.flag === null) continue;
      if (internalOnly.has(field)) continue;
      if (!actionInputs.has(def.input)) {
        missing.push(`${field} (input: "${def.input}")`);
      }
    }
    expect(
      missing,
      "field-schema entries with flags that lack an action.yml input",
    ).toEqual([]);
  });
});
