...
describe("checkEffortRejection", () => {
  ...
  it("returns silently when a non-OK error only mentions 'effort' as prose (no wire key)", async () => {
    // Given a 4xx whose message says "effort" but carries no `reasoning_effort`
    // / `reasoning.effort` / `output_config.effort` parameter.
    const response = new Response(
      JSON.stringify({ error: { message: "The effort you requested is not available for this model.", type: "invalid_request_error" } }),
      { status: 400 },
    );
    // When / Then — a prose mention must not be classified as an effort rejection.
    await expect(checkEffortRejection(response, { ...CONTEXT, secrets: [] })).resolves.toBeUndefined();
  });

  it("prefers the envelope error.message as the detail", async () => {
    // Given / When
    const message = await rejectionMessage(JSON.stringify({ error: { message: "effort level too high", param: "reasoning_effort" } }));
    // Then
  ...
  it("falls back to the raw body when the envelope record has no message field", async () => {
    // Given / When
    const message = await rejectionMessage(JSON.stringify({ error: { detail: "effort refused here", param: "reasoning_effort" } }));
    // Then
  ...
  it("uses a string envelope directly as the detail", async () => {
    // Given / When
    const message = await rejectionMessage(JSON.stringify({ error: "effort value not accepted", param: "reasoning_effort" }));
    // Then
  ...
  it("uses the raw body when the error response has no envelope", async () => {
    // Given / When
    const message = await rejectionMessage("effort rejected by gateway: reasoning_effort");
    // Then
  ...
  it("redacts sk- style tokens from the detail", async () => {
    // Given / When
    const message = await rejectionMessage(JSON.stringify({ error: { message: "effort bad; key sk-AbC123-x_y leaked", param: "reasoning_effort" } }));
    // Then
  ...
  it("redacts gh[pousr]_ style tokens from the detail", async () => {
    // Given / When
    const message = await rejectionMessage(JSON.stringify({ error: { message: "effort bad; token ghp_a1B2c3 and ghs_Z9y8x7 leaked", param: "reasoning_effort" } }));
    // Then
  ...
  it("redacts Bearer authorization strings from the detail", async () => {
    // Given / When
    const message = await rejectionMessage(JSON.stringify({ error: { message: "effort bad; Bearer opaque.token.value leaked", param: "reasoning_effort" } }));
    // Then
  ...
  it("replaces literal context secrets before regex redaction", async () => {
    ...
    const message = await rejectionMessage(
      JSON.stringify({ error: { message: "effort bad; key hunter2 leaked", param: "reasoning_effort" } }),
      { secrets: ["hunter2"] },
    );
    // Then
  ...
  it("collapses control characters before truncating the detail", async () => {
    // Given / When
    const message = await rejectionMessage(JSON.stringify({ error: { message: "effort badtrailing junk", param: "reasoning_effort" } }));
    // Then
