...
```bash
npx github:JosiahSiegel/umactually#v0.5.2 review
```

The `#v0.5.2` fragment pins the install to the tagged release. Omit the fragment only when you specifically want the latest unreleased `main` build. The `umactually` npm package is not yet published — `npm install -g umactually` will 404 until a future release.
...
CI must use Node.js 24 and a version-pinned install. Pin to the [`v0.5.2` release tag](https://github.com/JosiahSiegel/umactually/releases/tag/v0.5.0) — never track `main` and never use the interactive binary installers in a CI step.
