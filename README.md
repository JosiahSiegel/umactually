...
curl -fsSL https://github.com/JosiahSiegel/umactually/raw/main/scripts/install.sh | sh -s -- --tag v0.8.0
```
...
```bash
npx github:JosiahSiegel/umactually#v0.8.0 review
```

The `#v0.8.0` fragment pins to the tagged release. Omit it only when you specifically want the latest unreleased `main` build.
...
CI must use Node.js 24 and a version-pinned install. Pin to the [`v0.8.0` release tag](https://github.com/JosiahSiegel/umactually/releases/tag/v0.8.0) — never track `main` and never use the interactive binary installers in a CI step.
