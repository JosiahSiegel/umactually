 ...
 */
function extractAnthropicTextPayload(rawText, endpoint = ENDPOINT) {
...
async function runAnthropicRequest(config) {
    ...
    if (baseUrlCandidates.length > 1) {
        process.stderr.write(`::notice::UmActually Resolving Anthropic base URL: trying ${baseUrlCandidates.length} candidates in order: ${baseUrlCandidates.join(", ")}\n`);
    }
    let lastAttempt = {
        ok: false,
    ...
    for (const candidate of baseUrlCandidates) {
        ...
        const attempt = await anthropic_messages_runWithRetry(config, fetchImpl, requestId, candidate);
        if (attempt.ok) {
            return attempt;
        }
        // Auth failures, server errors, parse failures — none of these benefit
        // from trying a different URL shape, so return immediately.
        if (!anthropic_messages_isRoutableFailure(attempt.error)) {
            return attempt;
        }
        process.stderr.write(`::notice::UmActually Anthropic base URL ${candidate} returned routable failure (status=${attempt.error.status}); advancing to next candidate.\n`);
        lastAttempt = attempt;
    }
    return lastAttempt;
}
function anthropic_messages_isRoutableFailure(error) {
